...
def env_factory(path, state_est=True, mirror=False, speed=None, clock_based=False, simrate=60, **kwargs):
    ...
    """
    if path in ['Cassie-v0', 'CassieMimic-v0', 'CassieRandomDynamics-v0', 'CassieIK-v0', 'CassieIKNoDelta-v0', 'CassieStandingEnv-v0', 'CassieGroundFrictionEnv-v0', 'CassieNoDeltaNeutral', 'CassieSidestep']:
        from cassie import CassieEnv, CassieTSEnv, CassieIKEnv, UnifiedCassieIKEnv, UnifiedCassieIKEnvNoDelta, CassieEnv_nodelta
        from cassie import CassieEnv_rand_dyn, CassieEnv_speed_dfreq, CassieStandingEnv, CassieGroundFrictionEnv
        from cassie import CassieEnv_speed_no_delta_neutral_foot, CassieEnv_speed_sidestep
        ...
        elif path == 'CassieNoDeltaNeutral':
            env_fn = partial(CassieEnv_speed_no_delta_neutral_foot, "walking", simrate=simrate, clock_based=True, state_est=state_est)
        elif path == 'CassieSidestep':
            env_fn = partial(CassieEnv_speed_sidestep, "walking", simrate=simrate, clock_based=True, state_est=state_est)
        ...
        if mirror:
            ...
            if state_est:
                ...
                #   env_fn = partial(SymmetricEnv, env_fn, mirrored_obs=[0.1, 1, 2, 3, 4, -10, -11, 12, 13, 14, -5, -6, 7, 8, 9, 15, 
                #                                                     16, 17, 18, 19, 20, -26, -27, 28, 29, 30, -21, -22, 23, 24, 
                #                                                     25, 31, 32, 33, 37, 38, 39, 34, 35, 36, 43, 44, 45, 40, 41, 42, 46, 47], 
            ...
            env_fn = partial(SymmetricEnv, env_fn, mirrored_obs=mirror_obs, mirrored_act = [-5, -6, 7, 8, 9, -0.1, -1, 2, 3, 4])
        return env_fn
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        args.use_gae = False
        args.env_name = "CassieNoDeltaNeutral"
        args.policy_name = "fwrd_walk_StateEst_trajmatch"
        # args.name = "sidestep_StateEst_speedmatch_footytraj_doublestance_time0.4_land0.4_vels_avgdiff_simrate15_bigweight_actpenalty_retrain"#_pelaccel_hipyaw_footxypenalty"
        # args.name = "sidestep_StateEst_justfootmatch_simrate15_bigweight"
        args.logdir = "./logs/test/"
        index = ''
        while os.path.isfile(os.path.join("./trained_models/", args.policy_name + index + "_actor.pt")):
            if index:
                index = '_(' + str(int(index[2:-1]) + 1) + ')'
            else:
                index = '_(1)'
        args.policy_name += index
