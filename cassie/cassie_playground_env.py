...
class CassieEnv:
    def __init__(self, traj, simrate=60, clock_based=False, state_est=False):
        ...
        # add start locations for cassie playground
        self.start_locations = [(-1.0, 0, 1.01),
                                (9.5,  0, 1.01),
                                (24.5, 0, 1.01),
                                (39.5, 0, 1.01),
                                (46.0, 0, 0.31),
    ...
    def step_simulation(self, action):
        ...
        # currently using rate limiting, if no rate limiting replace real_action w/ target instead
    ...
    def step(self, action):
        ...
        print('Start Terrain')
        print('XML Size: ', self.sim.get_hfield_size()[0:4])
        print('Grid Size: {} x {}'.format(self.sim.get_hfield_nrow()[0], self.sim.get_hfield_ncol()[0]))
        print('Data: ', self.sim.get_hfield_data()[0:4])

        print('Step Terrain')
        print('XML Size: ', self.sim.get_hfield_size()[4:8])
        print('Grid Size: {} x {}'.format(self.sim.get_hfield_nrow()[1], self.sim.get_hfield_ncol()[1]))
        print('Data: ', self.sim.get_hfield_data()[4:8])

        print('Slope Terrain')
        print('XML Size: ', self.sim.get_hfield_size()[8:12])
        print('Grid Size: {} x {}'.format(self.sim.get_hfield_nrow()[2], self.sim.get_hfield_ncol()[2]))
        print('Data: ', self.sim.get_hfield_data()[8:100])
    ...
    def reset(self, location=None):
        ...
        # set start location for Cassie
        start_loc = self.start_locations[0]
        # start_loc = self.start_locations[location] if location is not None else random.choice(self.start_locations)
        self.sim.set_body_pos(start_loc)
    ...
    def reset_for_test(self, location=None):
        # set start location for Cassie
        start_loc = location if location else random.choice(self.start_locations)
        self.sim.set_body_pos(start_loc)
